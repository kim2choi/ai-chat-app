...
2. 신규 매수 결정 (현금 없어도 반드시 분석):
   - **중요**: 발굴된 모든 종목을 반드시 분석하고 평가할 것
   - 현금이 부족한 경우:
     * 옵션 A: 보유 종목 일부 매도 후 매수 (추천 순서 제시)
     * 옵션 B: 향후 현금 확보 시 매수 (우선순위 표시)
   - 각 발굴 종목에 대해:
     * 투자 매력도 (1-10점)
     * 추천 투자 금액 (현금 있다면)
     * 매수 타이밍 (즉시/관망)
...
3. 종목 평가 예시:
   - 발굴 종목 1: AAPL - 매력도 8/10, 추천 $500, 즉시 매수
   - 발굴 종목 2: TSLA - 매력도 6/10, 추천 $300, 관망
   
4. **절대 규칙**: 
   - "발굴 종목 없음"이라고 쓰지 말 것
   - 발굴된 종목이 있으면 반드시 분석 및 평가 제공
...
**실행 방안:**
- 현금 부족 시: [어떤 종목을 매도하여 현금 확보할지]
- 또는: 현금 확보 후 우선순위대로 매수
